...
<sup>¹ While illustrative of the point that one shouldn't add arbitrary lines in a chain of calls, this particular example suffers from another issue: a high complexity, leading to a suboptimal maintainability index. In production code, such chain should likely be refactored in multiple methods.</sup>
